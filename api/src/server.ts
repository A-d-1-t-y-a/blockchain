...
const awsIAMClient = new AWSIAMClient(process.env.AWS_REGION || "us-east-1");
...
app.post("/api/authorize", async (req: Request, res: Response) => {
  try {
    ...
    const authorized = awsDecision.allowed && (blockchainResult?.authorized ?? true);
...
app.get("/api/authorize/:requestId", async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ error: "Failed to get authorization", details: error.message });
  }
});
...
app.post("/api/policy/update-root", async (req: Request, res: Response) => {
  try {
    ...
    if (!blockchainClient) {
      return res.status(500).json({ error: "Blockchain client not configured" });
    }

    const txHash = await blockchainClient.updatePolicyRoot(newRoot);
    res.json({ success: true, transactionHash: txHash });
  } catch (error: any) {
    res.status(500).json({ error: "Policy update failed", details: error.message });
  }
});
